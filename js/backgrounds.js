class Backgrounds {
  constructor() {
    ...
    this._backgroundsList = document.querySelector(
      "#backgrounds-list.sidebar-settings-item-list"
    );
  ...
  _updateBackgroundsDefault() {
    ...
    let defaultAccountItem = document.querySelector(
      `#button-backgrounds-item-${id}`
    );
  ...
  _updateBackgroundVariables() {
    this._backgroundRandomMode =
      JSON.parse(this._localStorage.getItem("randomBackgroundImageMode")) ||
      false;
    this._backgroundCurrentPath =
      this._localStorage.getItem("defaultBackgroundImage") ||
      this._backgroundImages[1];
    if (this._backgroundRandomMode === false) {
      ...
      const randomIndex = this._randomIntegerForBackground(
        1,
        this._backgroundImages.length - 1
      );
      this._backgroundCurrentPath = this._backgroundImages[
  ...
  _randomItemSave() {
    this._localStorage.setItem(
      "randomBackgroundImageMode",
      JSON.stringify(this._backgroundRandomMode)
    );
  }
  ...
  _createBackgroundArray() {
    ...
    if (this._backgroundImagesDir) {
      this._backgroundImages =
        greeterutil.dirlist(this._backgroundImagesDir) || [];
    }
